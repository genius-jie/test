...
//zbbÌí¼Ó
function buildUserMap2($users,$add_options = false, $additional_options=null, $opt=null)
{
    $my['opt'] = array('userDisplayFormat' => null);
    $my['opt'] = array_merge($my['opt'],(array)$opt);
    ...
    if ($users)
    {
        ...
        {
            $userID = $userSet[$idx];
            $usersMap[$userID] = $users[$userID]->getDisplayName2($my['opt']['userDisplayFormat']);
            if($users[$userID]->isActive == 0)
            {
//                 $usersMap[$userID] = $inactivePrefix . ' ' . $usersMap[$userID];
                unset($usersMap[$userID]);
            }
        }
    }
    return $usersMap;
...
//zbbÌí¼Ó
function getTestersForHtmlOptions2(&$db,$tplanID,$tproject,$users = null,
    $additional_testers = null,$activeStatus = 'active')
{
    $orOperand = false;
    $activeTarget = 1;
    ...
    return buildUserMap2($userFilter,true,$additional_testers);
}
/**
 *
 */
