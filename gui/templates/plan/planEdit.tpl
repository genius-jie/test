...
{lang_get var="labels"
          s="warning,warning_empty_tp_name,testplan_title_edit,public,api_key,
             ...
             show_event_history,testplan_txt_notes,testplan_th_sprint_name"}
  ...
  <table class="common" width="80%">
    ...
    <tr><th style="background:none;">{$labels.testplan_th_name}</th>
      <td><input type="text" name="testplan_name"
       ...
       <!-- //创建计划的时候关联sprint --> 
	<tr><th style="background:none;">{$labels.testplan_th_sprint_name}</th>
      <td><input type="text" name="testplan_sprint_name"
                 size="{#TESTPLAN_NAME_SIZE#}"
                 maxlength="{#TESTPLAN_NAME_MAXLEN#}"
                 value="{$gui->testplan_sprint_name|escape}" />
          {include file="error_icon.tpl" field="testplan_name"}
      </td>
    </tr>
    ...
    {/if}
      <tr>
        <th style="background:none;">{$labels.testplan_th_active}</th>
          <td>
            <input type="checkbox" name="active" checked="checked"{if $gui->is_active eq 1}   {/if} />
          </td>
      </tr>
      <tr>
        <th style="background:none;">{$labels.public}</th>
          <td>
            <input type="checkbox" name="is_public" checked="checked" {if $gui->is_public eq 1} {/if} />
          </td>
      </tr>
