...
$art=$_SESSION['currentUser']->getDisplayName();
$mysqli = new mysqli("localhost", "root", "", "testlink");
$mysqli->query("SET NAMES UTF8");
$result=$mysqli->query("select * from users where users.login='".$art."';");
...
function initializeGui(&$dbHandler,$argsObj,&$tplanMgr,&$tcaseMgr)
{
  ...
  if ($argsObj->level != 'testproject')
  {
    ...
    //zbb��������һ��
    $gui->testers_firstname=getTestersForHtmlOptions2($dbHandler,$argsObj->tplan_id,$tproject_info,$gui->all_users);
  }
  //zbb����
  foreach ($gui->testers as $key=>$value){
      if($key!=0){
          $gui->login_name[$key]=$value.'('.$gui->testers_firstname[$key].')';
      }else{
          $gui->login_name[$key]='';
      }
  }
