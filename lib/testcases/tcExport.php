...
if($args->exportType=='XML'){
	...
	$gui->export_filename = is_null($args->export_filename) ? $gui->export_filename : $args->export_filename;
	$gui->export_filename=str_replace(".xml","",$gui->export_filename);	
}
//$gui->export_filename = iconv("utf-8","gb2312",$gui->export_filename);
if( $check_children )
{
...
$gui->export_filename=iconv("utf-8","gb2312",$gui->export_filename);
    ...
    switch($args->exportType)
  {
    ...
    case 'XLS':
		...
		$gui->export_filename=str_replace(".xml","",$gui->export_filename);
		exportExcel($arr,$gui->export_filename,$index);
		unset($arr);  
		unset($test);
		unset($Function_point);
		unset($index);
}
    exit();
  }
