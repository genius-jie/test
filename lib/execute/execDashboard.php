...
// print_r($tproject_mgr->get_by_id($args->tproject_id));
// echo "<br>";
// echo "<br>";
// echo lang_get("session_expired");
// $it_mgr = new tlIssueTracker($db);
// $issueTrackerCfg = $it_mgr->getLinkedTo($args->tproject_id);
// print_r($issueTrackerCfg);
$gui = initializeGui($db,$args,$cfg,$tplan_mgr, $its);
// echo "<br>";
// echo "<br>";
// echo "<br>";
...
*/
function initializeGui(&$dbHandler,&$argsObj,&$cfgObj,&$tplanMgr,&$itsObj)
{
  ...
  // iris 获取sprint的名称
  $gui->testplan_sprint_name = $itsObj->getSprintById($rs['sprint_id'])->name;
  $gui->testplan_sprint_name1 = $itsObj->getSprintById($rs['sprint_id'])->name;
  // Important note: 
  ...
  //iris 获取jira中的bugs列表
//   echo $rs['sprint_id'];
  list($mybugs,$mycount,$count)=$itsObj->getBugs($rs['sprint_id']);
  $gui->bugs=$mybugs;
  $gui->mycount=$mycount;
  $gui->bugR=$count;
  
  //iris 获取该计划版本的总用例数
  $gui->testcasecount=$tplanMgr->count_testcases($argsObj->tplan_id);
  $gui->testcaseR=$tplanMgr->getRC($argsObj->tplan_id);
  
  tLog($gui->bugR.'and'.$gui->testcaseR,"WARNING");
  return $gui;
}
