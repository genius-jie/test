...
// iris 初始化jira的数据库信息
list ($its, $issueTrackerCfg) = getIssueTracker($db, $args, $tproject_mgr);
...
if($args->do_action == "do_create" || $args->do_action == "do_update")
{
  ...
  // 张立杰 记录传递的sprint名称
  $gui->testplan_sprint_name = $args->testplan_sprint_name;
  ...
  // iris 判断sprint的名称在jira数据库是否存
  $sprint_exists = isSprintExist($its, $args->testplan_sprint_name);
...
switch($args->do_action)
{
  case 'fileUpload':
    ...
    //保证上传附件后，刷新后，测试计划里面的数据都还在
    getItemData($its,$tplan_mgr,$gui,$of,$args->tplan_id,true);
  break;
  ...
  case 'edit':
    getItemData($its,$tplan_mgr,$gui,$of,$args->tplan_id);
  break;
  ...
  case 'do_update':
    ...
    {//如果输入的sprint不存在或者为没输入
    	if (! $sprint_exists || $args->testplan_sprint_name == '') {
    		$gui->user_feedback = lang_get("warning_duplicate_sprint_name");
    	} else {
    		// iris将sprint的名称转成id方可存入数据库
    		$sprint = $its->getSprint($args->testplan_sprint_name);
    		// iris数据库更新
      if(!$tplan_mgr->update($args->tplan_id,$args->testplan_name,$args->notes,
                             $args->active,$args->is_public, $sprint->id))
      {
  ...
  case 'do_create':
    ...
    if(!$name_exists)
    {
    	// iris 判断sprint是否存在，如果存在，正常进行创建操作，如果不存在页面显示错误语句
    	if (! $sprint_exists || $args->testplan_sprint_name == '') {
    		$gui->user_feedback = lang_get("warning_duplicate_sprint_name");
    	} else {
    		// iris将sprint的名称转成id方可存入数据库
    		$sprint = $its->getSprint($args->testplan_sprint_name);
    		// iris 在数据库创建记录
    		$new_tplan_id = $tplan_mgr->create($args->testplan_name, $args->notes, $args->tproject_id, $args->active, $args->is_public, $sprint->id);
//     		$mysqli = new mysqli("10.7.12.4", "bitnami_testlink", "2c159fb359", "bitnami_testlink");
    		$mysqli = new mysqli("localhost", "root", "", "testlink");
    		$mysqli->query("SET NAMES UTF8");
    		$mysqli->query("insert into builds(testplan_id,name,notes,active,is_open)values($new_tplan_id,'V0.1-开发自测','开发自测',1,1);");
    		$mysqli->query("insert into builds(testplan_id,name,notes,active,is_open)values($new_tplan_id,'V1.0-冒烟测试','冒烟测试',1,1);");
    		$mysqli->query("insert into builds(testplan_id,name,notes,active,is_open)values($new_tplan_id,'V1.1-功能测试','功能测试',1,1);");
    		$mysqli->query("insert into builds(testplan_id,name,notes,active,is_open)values($new_tplan_id,'V1.1.1-场景测试','场景测试',1,1);");
    		$mysqli->query("insert into builds(testplan_id,name,notes,active,is_open)values($new_tplan_id,'V1.2-回归测试','回归测试',1,1);");
...
function init_args($request_hash)
{
  ...
  $nullable_keys = array('testplan_name','notes','rights','active','do_action','testplan_sprint_name');
 ...
 * 输入参数：jiradb,name
 */
function isSprintExist(&$itsobj, $name)
{
	$status = $itsobj->sprintIsExist($name);
	return $status;
}
 ...
 *
 */
function getItemData(&$itsobj,&$itemMgr,&$guiObj,&$ofObj,$itemID,$updateAttachments=false)
{
	//获得当前的测试计划
  ...
  if (sizeof($dummy))
  {
    ...
    $guiObj->testplan_sprint_name = $itsobj->getSprintById($dummy['sprint_id'])->name;
 ...
 * project对象
 */
function getIssueTracker(&$dbHandler, $argsObj, &$tprojectMgrObj)
{
	$its = null;
	$info = $tprojectMgrObj->get_by_id($argsObj->tproject_id);
