 ...
 * @uses testcase
 */
class tlTestCaseFilterControl2 extends tlFilterControl {
...
$id=$row['id'];
    ...
    $all_testers = getTestersForHtmlOptions($this->db, $tplan_id, $tproject_info, null,
                                          ...
                                          'any');//zbb：新增1728到1739行，修改1740行，对过滤器中的筛选拥有者显示成登录号(姓名)
    $all_testers_name = getTestersForHtmlOptions2($this->db, $tplan_id, $tproject_info, null,
                                           array(TL_USER_ANYBODY => $this->option_strings['any'],
                                                 TL_USER_NOBODY => $this->option_strings['none'],
                                                 TL_USER_SOMEBODY => $this->option_strings['somebody']),
                                           'any');
    foreach ($all_testers_name as $testkey => $testvalue) {
        if ($testkey <= 0) {
             $test_login_name[$testkey] = $testvalue;
         } else {
             $test_login_name[$testkey] = $all_testers[$testkey] . '(' . $testvalue . ')';
          }
      }
        $visible_testers = $test_login_name;
