...
class testplan extends tlObjectWithAttachments
{
   ...
   *     if problems -> 0.
   */
  function create($name,$notes,$testproject_id,$is_active=1,$is_public=1,$sprint_id)
  {
    ...
    $sql = "/* $debugMsg */ " . " INSERT INTO {$this->tables['testplans']} (id,notes,api_key,testproject_id,active,is_public,sprint_id) " .
	 " VALUES ( {$tplan_id} " . ", '" . $this->db->prepare_string($notes) . "'," .
	  "'" . $this->db->prepare_string($api_key) . "'," . 
	  $testproject_id . "," . $active_status . "," . $public_status . "
	  , '" . $this->db->prepare_string($sprint_id) . "')";
  ...
  function getRC($id){
  	...
  	$sql="/* $debugMsg */ "."SELECT EX.id AS ID ,TC.importance AS IMPORT, COUNT(DISTINCT NH.id) AS STEPNUM, EX.status AS STATUS"
  			." FROM {$this->tables['testplan_tcversions']} TPC JOIN {$this->tables['tcversions']} TC ON TC.id=TPC.tcversion_id"
  			." JOIN executions EX ON EX.tcversion_id=TPC.tcversion_id AND EX.testplan_id=TPC.testplan_id AND EX.platform_id=TPC.platform_id"
  					." LEFT JOIN nodes_hierarchy NH ON NH.parent_id=TPC.tcversion_id AND NH.node_type_id=9"
  							." WHERE TPC.testplan_id = '".$id."' "." GROUP BY TPC.id,EX.build_id";
  	...
  	foreach ($rs as $item){
  		if ($item['IMPORT']==1){
  			$count+=($item['STEPNUM']+1)*1;
  		}elseif ($item['IMPORT']==2){
  			$count+=($item['STEPNUM']+1)*2;
  		}elseif ($item['IMPORT']==3){
  			$count+=($item['STEPNUM']+1)*4;
  		}
  	}
  	return $count;
  }
   ...
   */
  function update($id,$name,$notes,$is_active=null,$is_public=null,$sprint)
  {
    $debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
    $do_update = 1;
    $result = null;
    ...
    if($do_update)
    {
      ...
      if($result)
      {
        ...
        if( !is_null($is_public) )
        {
         ...
         // iris 更新测试计划的备注和sprint
         $sql = " UPDATE {$this->tables['testplans']} " .
				" SET sprint_id='" . $this->db->prepare_string($sprint) . "' " . 
                ",notes='" . $this->db->prepare_string($notes) . "' " . " {$add_upd} WHERE id=" . $id;
        $result = $this->db->exec_query($sql);
      }
    }
    return ($result ? 1 : 0);
